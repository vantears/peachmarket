				...
				<div class="item">
					...
					<a href="<c:url value='/saleboard/detail?sb_num=${pr.sb_num}' />"
						...
						<div class="profile-date">
							...
							<div class="date">${pr.sb_date}</div>
						</div>
					</a>
				</div>
			</c:forEach>
			<c:if test="${dbBoardList.size() == 0 }">
				<div class="sc-kkbgRg krWbln">
					<div class="sc-cZBZkQ iKwFSh">
	...
	<ul class="pagination">
		<c:if test="${pm.prev}">
			<li class="page-item"><a class="page-link"
				href="<c:url value='/saleboard/${pm.cri.sc_num}${pm.cri.getUrl(pm.startPage-1)}'/>">이전</a>
			</li>
		</c:if>
			...
			<c:choose>
				<c:when test="${i == pm.cri.page}">
					<li class="page-item current ">
						<!-- 현재 페이지일 때 current 클래스 추가 --> <a class="page-link"
						href="<c:url value='/saleboard/${pm.cri.sc_num}${pm.cri.getUrl(i)}'/>">${i}</a>
					</li>
				</c:when>
				<c:otherwise>
					<li class="page-item"><a class="page-link"
		...
		<c:if test="${pm.next}">
			<li class="page-item"><a class="page-link"
				href="<c:url value='/saleboard/${pm.cri.sc_num}${pm.cri.getUrl(pm.endPage+1)}'/>">다음</a>
			</li>
		</c:if>
	</ul>
</body>
